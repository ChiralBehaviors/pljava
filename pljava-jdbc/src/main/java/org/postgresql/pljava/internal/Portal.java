...
public class Portal {
    ...
    private static native int _fetch(long pointer, long threadId,
                                     boolean forward, int count)
                                                                throws SQLException;

    private static native String _getName(long pointer) throws SQLException;

    private static native int _getPortalPos(long pointer) throws SQLException;

    private static native TupleDesc _getTupleDesc(long pointer)
                                                               throws SQLException;

    private static native boolean _isAtEnd(long pointer) throws SQLException;

    private static native boolean _isAtStart(long pointer) throws SQLException;

    private static native boolean _isPosOverflow(long pointer)
                                                              throws SQLException;

    private static native int _move(long pointer, long threadId,
                                    boolean forward, int count)
                                                               throws SQLException;
    ...
    /**
     * Invalidates this structure and frees up memory using the internal
     * function <code>SPI_cursor_close</code>
     */
    public void close() {
     ...
     * 
     * @param forward
     *            Set to <code>true</code> for forward, <code>false</code> for
     *            backward.
     * @param count
     ...
     */
    public int fetch(boolean forward, int count) throws SQLException {
        synchronized (Backend.THREADLOCK) {
            return _fetch(m_pointer,
     ...
     * using the {@link #close()} mehtod or implicitly due to a pop of
     * invocation context.
     */
     ...
     *             if the handle to the native structur is stale.
     */
    public int move(boolean forward, int count) throws SQLException {
        synchronized (Backend.THREADLOCK) {
            return _move(m_pointer,
                         System.identityHashCode(Thread.currentThread()),
                         forward, count);
        }
    }
