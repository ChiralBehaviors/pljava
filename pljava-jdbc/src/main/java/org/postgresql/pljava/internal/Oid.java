 ...
 * Should the size of that change from 32 bit, this class must change too. In
 * Java, the InvalidOid is represented as <code>null</code>.
 * 
 * @author Thomas Hallgren
 */
public class Oid extends Number {
    private static final HashMap<Class<?>, Oid> s_class2typeId   = new HashMap<Class<?>, Oid>();

    private static final HashMap<Oid, Class<?>> s_typeId2class   = new HashMap<Oid, Class<?>>();
    ...
    public static Oid forJavaClass(Class<?> clazz) {
        return s_class2typeId.get(clazz);
    }
    ...
    /**
     * A Type well known to PostgreSQL but not known as a standard XOPEN SQL
     * type can be registered here. This includes types like the Oid itself and
     * all the geometry related types.
     * 
     * @param clazz
     *            The Java class that corresponds to the type id.
     * @param typeId
    ...
    private static String getCanonicalClassName(String name, int nDims) {
        if (name.endsWith("[]")) {
            return getCanonicalClassName(name.substring(0, name.length() - 2),
                                         nDims + 1);
        }
    ...
    /**
     * Checks to see if the other object is an <code>Oid</code>, and if so, if
     * the native value of that <code>Oid</code> equals the native value of this
     * <code>Oid</code>.
     * 
