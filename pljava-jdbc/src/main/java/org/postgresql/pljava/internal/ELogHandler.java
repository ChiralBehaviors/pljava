...
public class ELogHandler extends Handler {
    ...
    /**
     * Informative messages that are always sent to client; is not affected by
     * client_min_messages
     */
    ...
    /**
     * Helpful messages to users about query operation; sent to client and
    ...
    public static Level getPgLevel() {
        ...
        if (pgLevel != null) {
            ...
            if (pgLevel.equals("panic") || pgLevel.equals("fatal")) {
                ...
                level = Level.FINER;
            } else if (pgLevel.equals("debug3") || pgLevel.equals("debug4")
                       || pgLevel.equals("debug5")) {
                level = Level.FINEST;
            }
        }
        return level;
    ...
    public void publish(LogRecord record) {
        ...
        Backend.log(pgLevel, getFormatter().format(record));
    }
    ...
    private void configure() {
        ...
        if (val != null) {
            try {
                setFilter((Filter) Class.forName(val.trim()).newInstance());
            } catch (Exception e) {
                val = null;
            }
        }
        ...
        if (val != null) {
            try {
                setFormatter((Formatter) Class.forName(val.trim()).newInstance());
            } catch (Exception e) {
                val = null;
            }
        }
