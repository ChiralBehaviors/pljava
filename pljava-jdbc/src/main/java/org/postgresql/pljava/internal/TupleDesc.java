...
public class TupleDesc extends JavaWrapper {
    private static native Tuple _formTuple(long _this, Object[] values)
                                                                       throws SQLException;

    private static native int _getColumnIndex(long _this, String colName)
                                                                         throws SQLException;

    private static native String _getColumnName(long _this, int index)
                                                                      throws SQLException;

    private static native Oid _getOid(long _this, int index)
                                                            throws SQLException;
     ...
     * @throws SQLException
     *             If the length of the values array does not match the size of
     *             the descriptor or if the handle of this descriptor has gone
     *             stale.
     */
    public Tuple formTuple(Object[] values) throws SQLException {
        synchronized (Backend.THREADLOCK) {
            return _formTuple(getNativePointer(), values);
        }
    }
     ...
     * @throws SQLException
     *             If no column with the given name can be found in this tuple
     *             descriptor.
     */
    public int getColumnIndex(String colName) throws SQLException {
        synchronized (Backend.THREADLOCK) {
            return _getColumnIndex(getNativePointer(), colName.toLowerCase());
        }
    }
    ...
    public String getColumnName(int index) throws SQLException {
        synchronized (Backend.THREADLOCK) {
            return _getColumnName(getNativePointer(), index);
        }
    }
    ...
    public Oid getOid(int index) throws SQLException {
        synchronized (Backend.THREADLOCK) {
            return _getOid(getNativePointer(), index);
        }
    }
