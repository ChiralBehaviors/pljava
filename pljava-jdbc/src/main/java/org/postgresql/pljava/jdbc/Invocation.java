...
public class Invocation {
    ...
    private ArrayList<PreparedStatement> m_preparedStatements;
    ...
    /**
     * Called from the backend when the invokation exits. Should not be invoked
     * any other way.
     */
        ...
        try {
            ...
            if (m_preparedStatements != null) {
                ...
                if (idx > 0) {
                    ...
                    w.warning("Closing " + idx + " \"forgotten\" statement"
                              + (idx > 1 ? "s" : ""));
                    while (--idx >= 0) {
                        PreparedStatement stmt = m_preparedStatements.get(idx);
                        w.fine("Closed: " + stmt);
                        stmt.close();
                    }
                }
    ...
    final void manageStatement(PreparedStatement statement) {
        if (m_preparedStatements == null) {
            m_preparedStatements = new ArrayList<PreparedStatement>();
        }
