...
public abstract class AbstractResultSetMetaData implements ResultSetMetaData {
    ...
    public final int getColumnDisplaySize(int column) throws SQLException {
        ...
        if (oid.equals(TypeOid.INT8) || oid.equals(TypeOid.NUMERIC)
            || oid.equals(TypeOid.FLOAT8) || oid.equals(TypeOid.OID)) {
            return 20;
        }
    ...
    public final boolean isCaseSensitive(int column) throws SQLException {
        checkColumnIndex(column);
        Oid oid = getOid(column);
        return oid.equals(TypeOid.TEXT) || oid.equals(TypeOid.BYTEA)
               || oid.equals(TypeOid.VARCHAR) || oid.equals(TypeOid.BPCHAR);
    }
    ...
    public final boolean isSigned(int column) throws SQLException {
        checkColumnIndex(column);
        Oid oid = getOid(column);
        return oid.equals(TypeOid.INT2) || oid.equals(TypeOid.INT4)
               || oid.equals(TypeOid.INT8) || oid.equals(TypeOid.FLOAT4)
    ...
    /**
     * Obtains connection to the database if needed. Once called, it stores the
     * connection reference to a private variable.
     */
