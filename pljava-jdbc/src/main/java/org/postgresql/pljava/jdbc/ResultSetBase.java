...
/**
 * A ResultSet base provides methods that are common both for a
 * SyntheticResultSet (which is not associated with a statement) and
 * SPIResultSet.
 * 
 * @author Filip Hrbek
 */
    ...
    public void setFetchSize(int fetchSize) throws SQLException {
        if (fetchSize <= 0) {
            throw new IllegalArgumentException(
                                               "Illegal fetch size for ResultSet");
        }
        m_fetchSize = fetchSize;
    }
