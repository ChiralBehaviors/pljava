...
public class SingleRowWriter extends SingleRowResultSet {
    ...
    public long getTupleAndClear() throws SQLException {
        ...
        //
        m_tuple = getTupleDesc().formTuple(m_values);
        Arrays.fill(m_values, null);
    ...
    public void updateObject(int columnIndex, Object x) throws SQLException {
        ...
        if (!c.isInstance(x) && !(c == byte[].class && x instanceof BlobValue)) {
            if (Number.class.isAssignableFrom(c)) {
                ...
                x = SPIConnection.basicCalendricalCoersion(c,
                                                           x,
    ...
    protected Object getObjectValue(int columnIndex) throws SQLException {
        if (columnIndex < 1) {
            throw new SQLException(
                                   "System columns cannot be obtained from this type of ResultSet");
        }
