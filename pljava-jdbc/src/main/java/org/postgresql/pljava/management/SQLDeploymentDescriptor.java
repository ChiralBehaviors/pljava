 ...
 * Subclause 5.2, &quot;&lt;token&gt;&quot; and &quot;&lt;separator&gt;&quot;, in ISO/IEC 9075-2.</code>
 * </pre>
 * 
...
public class SQLDeploymentDescriptor {
    ...
    private final ArrayList<String> m_installCommands = new ArrayList<String>();
    private final Logger            m_logger;
    private int                     m_position        = 0;

    private final ArrayList<String> m_removeCommands  = new ArrayList<String>();
    ...
    public void install(Connection conn) throws SQLException {
        executeArray(m_installCommands, conn);
    }
    ...
    public void remove(Connection conn) throws SQLException {
        executeArray(m_removeCommands, conn);
    }
    ...
    private void executeArray(ArrayList<String> array, Connection conn)
                                                                       throws SQLException {
        m_logger.entering("org.postgresql.pljava.management.SQLDeploymentDescriptor",
                          "executeArray");
        Session session = SessionManager.current();
        int top = array.size();
        for (int idx = 0; idx < top; ++idx) {
            String cmd = array.get(idx);
            m_logger.finer(cmd);
            session.executeAsSessionUser(conn, cmd);
        }
        m_logger.exiting("org.postgresql.pljava.management.SQLDeploymentDescriptor",
                         "executeArray");
    }
    ...
    private void readActionGroup() throws ParseException {
        m_logger.entering("org.postgresql.pljava.management.SQLDeploymentDescriptor",
                          "readActionGroup");
        this.readToken('"');
        if (!"BEGIN".equals(readIdentifier())) {
            throw parseError("Excpected keyword 'BEGIN'");
        }

        ArrayList<String> commands;
        String actionType = readIdentifier();
        if ("INSTALL".equals(actionType)) {
            commands = m_installCommands;
        } else if ("REMOVE".equals(actionType)) {
            commands = m_removeCommands;
        } else {
            throw parseError("Excpected keyword 'INSTALL' or 'REMOVE'");
        }
        ...
        for (;;) {
            ...
            //
            int top = cmd.length();
            if (top >= 15 && "BEGIN ".equalsIgnoreCase(cmd.substring(0, 6))
                ...
                if (cmd.charAt(pos) != ' ') {
                    throw parseError("Expected whitespace after <implementor name>");
                }
                ...
                if (!implementorName.equalsIgnoreCase(cmd.substring(endNamePos))
                    || !"END ".equalsIgnoreCase(cmd.substring(endPos,
                                                              endNamePos))) {
    ...
    private String readCommand() throws ParseException {
        ...
        if (inQuote != 0) {
            throw parseError("Untermintated " + (char) inQuote
    ...
    private String readIdentifier() throws ParseException {
        ...
        if (!Character.isJavaIdentifierStart(ch)) {
            throw parseError("Syntax error at '" + ch
                             + "', expected identifier");
        }
    ...
    private char readToken(char token) throws ParseException {
        int c = skipWhite();
        if (c < 0) {
            throw parseError("Unexpected EOF when expecting token '" + token
                             + '\'');
        }

        char ch = (char) c;
        if (ch != token) {
            throw parseError("Syntax error at '" + ch + "', expected '" + token
                             + "'");
        }
        return ch;
    }
    ...
    private char readToken(String tokens) throws ParseException {
        int c = skipWhite();
        if (c < 0) {
            throw parseError("Unexpected EOF when expecting one of \"" + tokens
                             + '"');
        }

        char ch = (char) c;
        if (tokens.indexOf(ch) < 0) {
            throw parseError("Syntax error at '" + ch + "', expected one of '"
                             + tokens + "'");
        }
        return ch;
    }
                ...
                //
                    ...
                    case '*':
                        ...
                        for (;;) {
                            ...
                            switch (c) {
                                case -1:
                                    throw parseError("Unexpected EOF when expecting end of multi line comment");
