 ...
 * deployer with the command: </p> <blockquote><code>
 ...
 * </p>
 * <h3>Deployer options</h3> <blockquote>
 * <table>
 * <th>Option</th>
 * <th>Description</th>
 * <tr>
 ...
 * <td>Use this option if the host runs on a windows platform. Affects the name
 * used for the Pl/Java dynamic library</td>
 * </tr>
 * </table>
 * </blockquote>
 * 
...
public class Deployer {
    ...
    private static final ArrayList<String> s_commands    = new ArrayList<String>();
    ...
    private static final int getCommand(String arg) {
        ...
        for (int idx = 0; idx < top; ++idx) {
            if (s_commands.get(idx).startsWith(arg)) {
    ...
    public void initializeSQLJSchema() throws SQLException {
        ...
        //
        stmt.execute("CREATE TABLE sqlj.typemap_entry("
        ...
        //
        stmt.execute("CREATE FUNCTION sqlj.install_jar(VARCHAR, VARCHAR, BOOLEAN) RETURNS void"
                     + "	AS 'org.postgresql.pljava.management.Commands.installJar'"
                     + "	LANGUAGE java SECURITY DEFINER");

        stmt.execute("CREATE FUNCTION sqlj.replace_jar(VARCHAR, VARCHAR, BOOLEAN) RETURNS void"
                     + "	AS 'org.postgresql.pljava.management.Commands.replaceJar'"
                     + "	LANGUAGE java SECURITY DEFINER");

        stmt.execute("CREATE FUNCTION sqlj.remove_jar(VARCHAR, BOOLEAN) RETURNS void"
        ...
        //
        stmt.execute("CREATE FUNCTION sqlj.install_jar(BYTEA, VARCHAR, BOOLEAN) RETURNS void"
                     + "	AS 'org.postgresql.pljava.management.Commands.installJar'"
                     + "	LANGUAGE java SECURITY DEFINER");

        stmt.execute("CREATE FUNCTION sqlj.replace_jar(BYTEA, VARCHAR, BOOLEAN) RETURNS void"
                     + "	AS 'org.postgresql.pljava.management.Commands.replaceJar'"
                     + "	LANGUAGE java SECURITY DEFINER");
        ...
        // to be optional.
        //
        stmt.execute("CREATE FUNCTION sqlj.set_classpath(VARCHAR, VARCHAR) RETURNS void"
                     + "	AS 'org.postgresql.pljava.management.Commands.setClassPath'"
                     + "	LANGUAGE java SECURITY DEFINER");

        stmt.execute("CREATE FUNCTION sqlj.get_classpath(VARCHAR) RETURNS VARCHAR"
        ...
        //
        stmt.execute("CREATE FUNCTION sqlj.add_type_mapping(VARCHAR, VARCHAR) RETURNS void"
                     + "	AS 'org.postgresql.pljava.management.Commands.addTypeMapping'"
                     + "	LANGUAGE java SECURITY DEFINER");

        stmt.execute("CREATE FUNCTION sqlj.drop_type_mapping(VARCHAR) RETURNS void"
    ...
    public void initJavaHandlers() throws SQLException {
        Statement stmt = m_connection.createStatement();
        stmt.execute("CREATE FUNCTION sqlj.java_call_handler()"
                     + " RETURNS language_handler" + " AS 'pljava'"
                     + " LANGUAGE C");

        stmt.execute("CREATE TRUSTED LANGUAGE java HANDLER sqlj.java_call_handler");

        stmt.execute("CREATE FUNCTION sqlj.javau_call_handler()"
                     + " RETURNS language_handler" + " AS 'pljava'"
                     + " LANGUAGE C");
